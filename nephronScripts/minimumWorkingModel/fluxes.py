...
def fluxes(x, param, ExpType, w = [1., 1., 1., 1.], potassiumW = 1., DCA = 1., cana = 1.):
    ...
    x_C1 = param[30] * 0.75
    x_C3 = param[31] * 0.50 # 0.516 From Benard et al., 2006
    x_C4 = param[32] * 0.25 # 0.264 From Benard et al., 2006
    x_F1 = param[33]
    ...
    #J_gdh = cana*0.31*(0.02)*GLU_x*(NAD_x/6.06e-5)/(1.9e-3+GLU_x)
    #if NAD_x < MinCon or GLU_x < MinCon:
    #    J_gdh = 0.
    ## 0.31 ratio of kidney to liver activity
    J_gdh = 0.0
    ...
    A = max([ATP_c, 0.]) * (Mg_c / Km[pcIR.iATP]) / P_c[pcIR.iATP]
    B = GLC_c
    P = G6P_c
    ...
    if (ExpType == 2) or (ExpType == 5):
        ...
        if ATP_c1 < 0 or PCr_c < 0:
            J_CKe = x_CK * (K_CK * ADP_c1 * max([PCr_c, 0.]) * H_c -
                            max([ATP_c1, 0.]) * Cr_c)
    ...
    if (ExpType == 2) or (ExpType == 5):
        ...
        J_AKe = x_AK * (Kapp_AKc * ADP_c * ADP_c - AMP_c * max([ATP_c, 0.]))
