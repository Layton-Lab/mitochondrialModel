...
def iterate(i, perturb): ## Runs differential equation for time span and outputs results to
    ## a csv file and a feather file.
    results = sci.solve_ivp(fun = f,
                            t_span = (0, 1000),
                            y0 = pc.finalConditions+perturb,
                            method = "LSODA"
                            )
    ...
    results = pd.DataFrame(results,
                           columns = ["t", "H_x", "dPsi", "ATP_x", "ADP_x",
                                      ...
                                      "PCr_c", "AMP_c"])
    results.to_csv("../results/perturb/results"+str(i)+".csv")
    feather.write_dataframe(results, "../results/perturb/resultsFe"+str(i)+".feather")

def main():
    linearizationReal = pd.read_csv("../results/linearizationReal.csv",
                                    header = None)

    print(linearizationReal)
