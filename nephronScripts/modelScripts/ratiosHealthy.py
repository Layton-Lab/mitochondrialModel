...
def main():
    ...
    for i in range(5):
        for j in range(3):
            if i == 2 & j==2:
                dat = pd.read_feather("../results/resultsFe.feather")\
                    .tail(1).values[0]
                dat = np.delete(dat, 0)
            else:
                dat = pd.read_feather("../results/resultsIO"+str(i)+str(j)+
    ...
    feather.write_dataframe(results, "../results/ratiosHealthy.feather")
