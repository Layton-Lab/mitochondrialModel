...
## In order to run this file you should first run the script potassiumTAL.py

readfn <- function(x) lapply(x, function(x) {data.table::fread(x)})
lowK <- paste0("../results/resultsTALNigericinKLeak", 1:10, "K1.csv")
normalK <- paste0("../results/resultsTALNigericin", 1:10, ".csv")
highK <- paste0("../results/resultsTALNigericinKLeak", 1:10, "K3.csv")
...
pdf("../dataVis/kleakKHAntiporterTAL.pdf")
...
## Figure 3.8
pdf("../dataVis/kleakKHAntiporterAbsTAL.pdf")
par(cex.axis = 1.5, cex.lab = 1.5)
