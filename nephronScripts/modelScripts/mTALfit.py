...
nadtotnorm = pc.pcPC.NADtot
W_m = pc.pcPC.W_m  ## Have to calculate W_x and W_i anew every time we chane
## the W_m (see pcPC in pc.py)
W_i = pc.pcPC.W_i
W_x = pc.pcPC.W_x
...
def main(): ## Runs differential equation for time span and outputs results to
    ...
    for i in range(7):
        ...
        if i == 0:
            pc.ics[pc.pcIS.iO2_x] = o2norm/4.0
        if i == 1:
            pc.ics[pc.pcIS.iO2_x] = o2norm/2.0
        if i == 2:
            pc.ics[pc.pcIS.iO2_x] = o2norm*2.0
        if i == 3:
            pc.pcPC.W_m = 0.568
            pc.pcPC.W_x = 0.9*0.568
            pc.pcPC.W_i = 0.1*0.568
        if i == 4:
            pc.ics[pc.pcIS.iO2_x] = o2norm/4.0
        ...
        if i == 5:
            ...
            pc.params[38] = hleaknorm*10.0
        if i == 6:
            pc.pcPC.NADtot = nadtotnorm*10.0
        ...
        results = pd.DataFrame(results,
                            columns = ["t", "H_x", "dPsi", "ATP_x", "ADP_x",
                                      ...
                                      "PCr_c", "AMP_c"])
        results.to_csv("../results/resultsmTALfit"+str(i)+".csv")
