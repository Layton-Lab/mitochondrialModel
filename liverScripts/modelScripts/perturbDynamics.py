...
O2 = pc.livFinalConditions[pc.pcIS.iO2_x]

def f(t, y): ## Differential equations, with oxygen oscillations
    print(t)
    a = equations.conservationEqs(y, J_AtC = J_AtC,
                              ExpType = ExpType,
                              StateType = StateType,
                              glyc = glyc)
    a[pc.pcIS.iO2_x] = -0.5*O2*np.sin(0.25*t)
    return a

PYRC = pc.livFinalConditions[pc.pcIS.iPYR_c]

def g(t, y): ## Differential equations, with cytosolic pyruvate oscillations
    print(t)
    a = equations.conservationEqs(y, J_AtC = J_AtC,
                              ExpType = ExpType,
                              StateType = StateType,
                              glyc = glyc)
    a[pc.pcIS.iPYR_c] = PYRC*np.cos(0.25*t)
    return a

def h(t, y): ## Differential equations, with cytosolic pyruvate oscillations
    print(t)
    a = equations.conservationEqs(y, J_AtC = J_AtC*np.cos(0.2*t),
                              ExpType = ExpType,
                              StateType = StateType,
                              glyc = glyc)
    return a
...
def main(): ## Runs differential equation for time span and outputs results to
    ...
    results = pd.DataFrame(results,
                           columns = ["t", "H_x", "dPsi", "ATP_x", "ADP_x",
                                      ...
                                      "PCr_c", "AMP_c"])
    results.to_csv("../results/resultsPerturbO2.csv")
    print(results)
    ...
    results = pd.DataFrame(results,
                           columns=["t", "H_x", "dPsi", "ATP_x", "ADP_x",
                                    ...
                                    "FUM_c", "ICIT_i", "ICIT_c", "GLC_c", "G6P_c",
                                    "PCr_c", "AMP_c"])
    results.to_csv("../results/resultsPerturbPYR.csv")
    print(results)
    ...
    results = pd.DataFrame(results,
                           columns=["t", "H_x", "dPsi", "ATP_x", "ADP_x",
                                    ...
                                    "FUM_c", "ICIT_i", "ICIT_c", "GLC_c", "G6P_c",
                                    "PCr_c", "AMP_c"])
    results.to_csv("../results/resultsPerturbATP.csv")
    print(results)
    return results
