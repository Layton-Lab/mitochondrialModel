...
def conservationEqs(x, J_AtC, ExpType = 1, StateType = 1,
                    ...
                    capacitanceWeight = 1., DCA = 1.,
                    potassiumW = 1., glyc = 0.0, t=1,
                    params = params):
    ...
    Jsol1 = fl.fluxes(x, params, ExpType = ExpType, w = w,
                potassiumW = potassiumW)

    J_AtC = 1.25*J_AtC*(x[pcIS.iATP_c]/(1.486e-3+x[pcIS.iATP_c]))
    glyc = glyc*(x[pcIS.iADP_c]/(1e-6+x[pcIS.iADP_c]))
    J_AtC = J_AtC-glyc
    
    ## Changed to maintain postive invariance of ATP_c, which appears to be a
    ## source of problems for the model in the kidney.  This should not change the normal
    ...
    ## concentration range is similar to predictions from experiment.

    ## Experimentally-dependent parameters
    if ExpType == 1:
        W_c = 0.77 # cytoplasm water fraction - LeFurgey et al. 1991
        Vmito = 0.230 # (ml mito / ml cell)
        Vcyto = 0.700 # (ml cyto / ml cell)
        ...
        W_c = 0.77 # cytoplasm water fraction - LeFurgey et al. 1991
        Vmito = 0.345 # (ml mito / ml cell)
        Vcyto = 0.458 # (ml cyto / ml cell)
        Rm_cyto = Vmito / Vcyto # Volume ratio mito volume / cytoplasm volume
        Rm_cell = Vmito # Volume ratio mito volume / cell volume
        Rc_cell = Vcyto # Volume ratio of cytosol / cell volume
    else:
    ...
    f[pcIS.iNADH_x] = (+DCA*Jsol1[pcFl.J_pdh] + Jsol1[pcFl.J_isod] + Jsol1[pcFl.J_akgd] +
        Jsol1[pcFl.J_mdh] - Jsol1[pcFl.J_C1] + Jsol1[pcFl.J_gdh])/pcPC.W_x #  + Jsol1[pcFl.J_gdh]
    f[pcIS.iQH2_x]  = (+Jsol1[pcFl.J_sdh] + Jsol1[pcFl.J_C1] - Jsol1[pcFl.J_C3])/pcPC.W_x
    f[pcIS.iPYR_x]  = (-DCA*Jsol1[pcFl.J_pdh] + Jsol1[pcFl.J_PYR_H])/pcPC.W_x
    f[pcIS.iACCOA_x] = (-Jsol1[pcFl.J_cits] + DCA*Jsol1[pcFl.J_pdh])/pcPC.W_x
    ...
    f[pcIS.iAKG_x]  = (+Jsol1[pcFl.J_isod] - Jsol1[pcFl.J_akgd] - Jsol1[pcFl.J_got] +
        Jsol1[pcFl.J_AKG_MAL] + Jsol1[pcFl.J_gdh])/pcPC.W_x # + Jsol1[pcFl.J_gdh]
    f[pcIS.iSCOA_x] = (+Jsol1[pcFl.J_akgd] - Jsol1[pcFl.J_scoas])/pcPC.W_x
    f[pcIS.iCOASH_x]  = (-DCA*Jsol1[pcFl.J_pdh] - Jsol1[pcFl.J_akgd] + Jsol1[pcFl.J_scoas]
    ...
    f[pcIS.iGLU_x]  = (+Jsol1[pcFl.J_got] + Jsol1[pcFl.J_GLU_H]
        - Jsol1[pcFl.J_ASP_GLU] - Jsol1[pcFl.J_gdh])/pcPC.W_x # - Jsol1[pcFl.J_gdh]
    ...
    # PHI's:
    Phi_H = - np.sum( H_x*dxxdt/(Kh*P_x) ) + 1*(-1*DCA*Jsol1[pcFl.J_pdh] +
